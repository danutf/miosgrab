...
static void set_filename(char *fname, char *out_dir){
	time_t now;
	struct tm *tm_now;
	struct timeval te;
	long long ts;
	char date_dir[12 + strlen(out_dir)];
	struct stat st = {0};
	...
	ts = te.tv_sec*1000LL + te.tv_usec/1000;

	sprintf(date_dir, "%s/%4d%02d%02d", out_dir, tm_now->tm_year+1900, tm_now->tm_mon+1, tm_now->tm_mday);
	
	//printf("timestamp=%llu, dir=%s\n", ts,date_dir);

	if(stat(date_dir, &st) == -1){	
		mkdir(date_dir, S_IRWXU | S_IRWXG);
	}

	sprintf(fname, "%s/%lu_%03lu.ppm", date_dir, now, te.tv_usec/1000);
			...
			int n_frames, char *out_dir)
{
	...
	fd_set				fds;
	FILE				*fout;
	char				out_name[50 + strlen(out_dir)];
...
const char *argp_program_version = "MIOS grabber version 1.0";
const char *argp_program_bug_address = "Dan Firac <dan.firac@gmail.com>";
